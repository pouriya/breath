...
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/breath.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
...
shell: shell-compile
	...
	$(PRE) erl -pa `ls -d _build/default/lib/*/ebin` \
                   ...
                   -eval "begin application:load('breath'), code:load_file('breath'), case application:get_all_key('breath') of {ok, K} -> lists:foreach(fun code:load_file/1, proplists:get_value(modules, K, [])); _ -> ok end end" \
                   +B
